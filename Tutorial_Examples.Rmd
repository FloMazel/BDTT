...
###### metric:
Beta-diversity metric chosen; defaut is Jaccard ("jac") and Bray-Curtis ("bc"). 
...
##### The GetHnodes function 
cmpute the "height" of each ndoe in the phylogeny defined as the maximum evolutionnary distance between this node and its descendant tips. 
...
for (i in as.character(slices))
{
   ...
   res=unlist(adonis(formula =Betas[i,"bc",samples,samples]~Meta[samples])$aov.tab[1,c(6,5,4)])
   StatsRes[(StatsRes$metric=="bc")&(StatsRes$similarity_slices==i),4:6]=res
}
```
